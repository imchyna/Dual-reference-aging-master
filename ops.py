...
def conv2d(input_map, num_output_channels, size_kernel=5, stride=2,reuse=False, name='conv2d'):
...
def fc(input_vector, num_output_length, reuse=False, name='fc'):
    with tf.variable_scope(name,reuse=reuse):
...
def deconv2d(input_map, output_shape, size_kernel=5, stride=2, stddev=0.02,reuse=False, name='deconv2d'):
    with tf.variable_scope(name,reuse=reuse):
...
def load_image(
    ...
    if save:
        im_name = os.path.join(save_path,image_path.split('/')[3])
...
def save_seperate_images(
    ...
    for ind, image in enumerate(images):
        im_name = os.path.join(save_path,str(ind//6)+'-'+str(ind)+'.png')
        imsave(im_name, image)

def save_sample_file(batch_images,save_path):
    for i,i_img in enumerate(batch_images):
        img = imread(i_img)
        savename = os.path.join(save_path,str(i_img).split('/')[3])
        imsave(savename,img)


def siamese_cosine_loss(x, y):
    dot_xy = tf.diag_part(tf.tensordot( x, y, axes=[[1],[1]]))

    sx =tf.reduce_sum(tf.square(x), axis=1)
    sy = tf.reduce_sum(tf.square(y), axis=1)

    return dot_xy/tf.multiply(tf.sqrt(sx),tf.sqrt(sy))
